...
class PromptEditService:
    ...
    @transaction.atomic
    def edit_by_prompt(self, image_base64: str, prompt: str, user_id: int = None) -> Dict[str, Any]:
        """
        根据描述修改图片
        ...
        task = None
        try:
            logger.info(f"开始描述修改，用户ID: {user_id}, 描述: {prompt[:50]}...")
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                logger.info(f"描述修改成功，URL: {image_url}")
                
                # 下载并保存图片
                saved_info = self._download_and_save_image(image_url, user_id, 'prompt')
                ...
                if task:
                    ...
                    GeneratedImage.objects.create(
                        ...
                        orig_prompt=f'描述修改：{prompt}',
                        download_time=timezone.now(),
                        is_downloaded=True
                    )
                    
                    logger.info(f"描述修改图片已保存到数据库，任务ID: {task_id}")
                ...
                error_msg = f"描述修改失败: {response.code} - {response.message}"
    ...
    def _download_and_save_image(
        ...
        try:
            ...
            logger.info(f"描述编辑图片保存成功: {saved_path}")
...
# 创建服务实例
prompt_edit_service = PromptEditService()

...
# 使用统一的StandardResponse，移除旧的ApiResponse类
...
class LoginView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            }, "登录成功", request_id=getattr(request, 'request_id', None))
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def me(self, request):
        ...
        if request.method == 'GET':
            ...
            return StandardResponse.success(serializer.data, request_id=getattr(request, 'request_id', None))
            ...
            if serializer.is_valid():
                serializer.save()
                return StandardResponse.success(serializer.data, "用户信息更新成功", request_id=getattr(request, 'request_id', None))
            print(f"序列化器错误: {serializer.errors}")
            return StandardResponse.error("用户信息更新失败", code=400, data=serializer.errors, request_id=getattr(request, 'request_id', None))
    ...
    def upload_avatar(self, request):
        ...
        return StandardResponse.success(serializer.data, "头像上传成功", request_id=getattr(request, 'request_id', None))
    ...
    def change_password(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return StandardResponse.success(None, "密码修改成功", request_id=getattr(request, 'request_id', None))
